...
// accessAllowingAllProviders builds the access a caller permitted every provider carries: the
// provider permits it holds are overrides, not the whole of what it may reach, so the deployment's
// configured set is what completes it.
func accessAllowingAllProviders(configured []string, permits ...schemas.ProviderPermit) schemas.Access {
	...
	return grant.NewAccess([]schemas.Permit{permit}, nil, "", nil,
		grant.WithConfiguredProviders(func() []string { return configured }))
}
